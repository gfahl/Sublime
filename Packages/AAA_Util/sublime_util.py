...
def is_rectangular(self, v):
    if len(set([(v.col(rg.begin()), v.col(rg.end())) for rg in self])) != 1:
        ...
        return False
    if v.col(rg.begin()) == v.col(rg.end()):
        # width must be >= 1
        return False
    if list(set([v.row(rg.end()) - v.row(rg.begin()) for rg in self])) != [0]:
        # no region can span more than one row
        return False
    if range(v.row(self[0].begin()), v.row(self[-1].begin()) + 1, 1) != [v.row(rg.begin()) for rg in self]:
        # rows should be consecutive
        return False
    return True

sublime.RegionSet.is_rectangular = is_rectangular
